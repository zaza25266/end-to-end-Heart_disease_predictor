...
def train_and_log_models():
    ...
    with mlflow.start_run(run_name="Tuned_LightGBM"):
        ...
        grid_search_LGBM = GridSearchCV(LGBM_pipeline, param_grid_LGBM, cv=5, n_jobs=-1)
        grid_search_LGBM.fit(X_train, y_train)
        best_LGBM_model = grid_search_LGBM.best_estimator_
        y_pred_LGBM = best_LGBM_model.predict(X_test)

        mlflow.log_params(grid_search_LGBM.best_params_)
        log_matrices(y_test, y_pred_LGBM, model_name="Tuned_LightGBM")
        mlflow.sklearn.log_model(best_LGBM_model, "Tuned_lightgbm_model")
        print("logged lightgbm model and metrics to mlflow")
    ...
    with mlflow.start_run(run_name="Tuned_XGBoost"):
        ...
        grid_search_XGB = GridSearchCV(XGB_pipeline, param_grid_XGB, cv=5, n_jobs=-1)
        grid_search_XGB.fit(X_train, y_train)
        best_XGB_model = grid_search_XGB.best_estimator_
        y_pred_XGB = best_XGB_model.predict(X_test)

        mlflow.log_params(grid_search_XGB.best_params_)
        log_matrices(y_test, y_pred_XGB, model_name="Tuned_XGBoost")
        mlflow.sklearn.log_model(best_XGB_model, "Tuned_xgboost_model")
        print("logged xgboost model and metrics to mlflow")
